...
def main(
    *,
    ...
    print(f"packed_int_capture_ok={packed_ok}")
    if packed_err is not None:
        print(f"packed_int_capture_error={packed_err}")
    print(f"int8_per_token_head_capture_ok={int8_ok}")
    if int8_err is not None:
        print(f"int8_per_token_head_capture_error={int8_err}")


if __name__ == "__main__":
    parser = FlexibleArgumentParser(
        description=(
            "Validate whether packed-int and int8 Triton decode paths are "
            "CUDA-graph capture safe on the same static shape."
        )
    )
    parser.add_argument(
        "--path",
